...
stop:
  ...
  # require_session_summary: true   # DB mode only — not generated by default
  max_retries: 8
```
...
**`require_session_summary`** (DB mode only, not generated by default) — ensures
a session summary is saved to the database before exit, preserving continuity
across sessions. Enable manually when using SQLite or PostgreSQL.
